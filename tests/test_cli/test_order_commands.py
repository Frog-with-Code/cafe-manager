...
def mock_context(mocker):
    mocker.patch(f"{PATCH_TARGET}.init_context")
    mocker.patch(f"{PATCH_TARGET}.get_env_path", return_value=Path("fake.db"))
    mocker.patch(f"{PATCH_TARGET}.SQLiteOrderRepo")
    mocker.patch(f"{PATCH_TARGET}.SQLiteInventoryRepo")
    mocker.patch(f"{PATCH_TARGET}.SQLiteMenuRepo")
    ...
    mocker.patch(f"{PATCH_TARGET}.SQLiteClientRepo")
    mocker.patch(f"{PATCH_TARGET}.SQLiteEmployeeRepo")
    mocker.patch(f"{PATCH_TARGET}.SQLiteCoffeeMachineRepo")

class TestCreateCommand:
    def test_create_success(self, mocker, mock_context):
    ...
    def test_create_invalid_format(self, mocker, mock_context):
        result = runner.invoke(app, ["create", "invalid_item"])
        assert result.exit_code != 0
        assert "Format invalid_item is incorrect" in result.stderr

    def test_create_insufficient_stocks(self, mocker, mock_context):
        mock_handler = mocker.patch(f"{PATCH_TARGET}.OrderCreateHandler")
        mock_handler.return_value.handle.side_effect = InsufficientStocksError("No beans")
...
class TestPayCommand:
    def test_pay_success(self, mocker, mock_context):
    ...
    def test_pay_order_not_found(self, mocker, mock_context):
        mock_handler = mocker.patch(f"{PATCH_TARGET}.OrderPayHandler")
    ...
    def test_pay_account_error(self, mocker, mock_context):
        mock_handler = mocker.patch(f"{PATCH_TARGET}.OrderPayHandler")
        mock_handler.return_value.handle.side_effect = AccountNotFoundError("Bank error")
        
        result = runner.invoke(app, ["pay", "--id", "id", "--price", "10", "--account", str(uuid4())])
        
        assert result.exit_code == 1
        assert "Bank error" in result.stderr

class TestInfoCommand:
    def test_info_basic(self, mocker, mock_context):
    ...
    def test_info_expanded(self, mocker, mock_context):
        mock_handler = mocker.patch(f"{PATCH_TARGET}.OrderInfoHandler")
        mock_order = mocker.MagicMock()
...
class TestServeCommand:
    def test_serve_success(self, mocker, mock_context):
    ...
    def test_serve_invalid_state(self, mocker, mock_context):
        mock_handler = mocker.patch(f"{PATCH_TARGET}.OrderServeHandler")
